...
def inject_xray_data(epub_path, json_path, output_epub_path):
    ...
    # Dynamically detect the folder where chapters live
    chapter_folder = None
    for item in book.get_items_of_type(ITEM_DOCUMENT):
        name = item.get_name()
        if '/' in name:
            chapter_folder = name.rsplit('/', 1)[0]
        break

    print(f"Detected chapter folder: {chapter_folder}")
    ...
    footnote_filename = (
        f'{chapter_folder}/xray_footnotes.xhtml'
        if chapter_folder
        else 'xray_footnotes.xhtml'
    )

    footnote_item = epub.EpubHtml(
        uid='xray_footnotes',
        title='X-Ray Summaries',
        file_name=footnote_filename,
        lang='en'
    )
    ...
    for item in book.get_items_of_type(ITEM_DOCUMENT):
        ...
        def dynamic_replace(match):
            ...
            return (
                f'<a href="xray_footnotes.xhtml#{safe_id}" epub:type="noteref" '
    ...
    for spine_item in book.spine:
        if isinstance(spine_item, tuple):
            ...
            item = spine_item
        if item and item.title is None:
            item.title = item.get_name()
